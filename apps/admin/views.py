...
def stop_signal(request):
    """
    Endpoint llamado por el reproductor de la sede (cliente) para marcar la sede como inactiva
    cuando la canción deja de reproducirse.
    """
    try:
        data = json.loads(request.body.decode('utf-8'))
        sede_id = data.get('sede_id')
        ...
        logging.error(f"Error al marcar sede inactiva: {e}")
        return JsonResponse({'status': 'error', 'message': str(e)}, status=500)
